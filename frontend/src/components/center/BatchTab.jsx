...
import { Database, FileUp, Loader2, Play, PlusCircle } from 'lucide-react';
      ...
      <div className="flex flex-col space-y-2">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            {/* Inline Ingest Actions */}
            <div className="flex items-center gap-2 ml-4 pl-4 border-l border-slate-200">
              <button 
                onClick={() => setIsDbModalOpen(true)}
                className="flex items-center gap-1.5 px-2 py-1 bg-white border border-slate-200 rounded hover:border-slate-400 transition-all group"
                title="Load from Database"
              >
                <Database size={13} className="text-slate-400 group-hover:text-slate-900 transition-colors" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500 group-hover:text-slate-900">Database</span>
              </button>
              ...
              >
                <FileUp size={13} className="text-slate-400 group-hover:text-slate-900 transition-colors" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500 group-hover:text-slate-900">Files</span>
              </button>
            </div>
          </div>
        ...
        <textarea 
          ...
          className="nexus-tool-textarea w-full h-40 resize-none border-2 border-black p-3 font-mono text-xs focus:outline-none bg-slate-50/30"
          placeholder="Paste multiple HL7 v2 messages here, or use the ingestion tools above..."
        />
      </div>
        ...
        <div className="nexus-tool-panel-body flex-1 overflow-y-auto p-3 bg-slate-50">
          {results.length > 0 ? (
            ...
            <div className="nexus-tool-empty h-full flex items-center justify-center font-mono text-[10px]">
              {isProcessing ? 'Streaming validation results...' : 'Awaiting batch stream'}
            </div>
          )}
        </div>
